...
def main():
    ...
    model = xgb.XGBRanker(
        ...
        objective="rank:ndcg",
        eval_metric="ndcg",   
        n_estimators=12000, # 10k or 12k works best
        learning_rate=0.05,
        max_depth=10, # 8 or 10 works best 
        subsample=0.8,
    ...
    model.fit(
        ...
        eval_group = [grp_val],           
        verbose=50,                          # log every 50 boosting rounds
    )
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    model_path = os.path.join(model_dir, f"xgb_regressor_{timestamp}.json") # Note: XGBoost uses .json format 
